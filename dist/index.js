...
const defaultValue = {
  label: ''
}; // Used as return for empty array
...
class QuickSearch extends Component {
  ...
  render() {
    ...
    const rows = items.map((item, index) => {
      ...
      if (queryPosition === -1) {
        ...
        labelComponent = h("span", null, label);
      } else {
        ...
        labelComponent = h("span", null, first, h(HighlightComponent_, itemProps, second), third);
      }

      return h(ItemComponent_, Object.assign({
        key: item.value
      }, itemProps), h(IndicatorComponent_, itemProps), labelComponent, !isLast && h("br", null));
    });
    return h("span", null, rows, h(StatusComponent_, {
      hasMatch: this.state.hasMatch
    }, h("br", null), this.state.query));
  }
  ...
  handleKeyPress(ch, key) {
    ...
    } else if (hasAnsi(key.sequence)) {// No-op
    } else {
